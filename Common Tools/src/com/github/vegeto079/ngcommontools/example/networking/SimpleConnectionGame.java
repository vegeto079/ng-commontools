...
public class SimpleConnectionGame extends Game {
	static SimpleConnectionGame us = null;
	Server server;
	Client client;
	int ourPos = -1;
	...
	long connectionSpeed = 10;
	String display = "";
	String ip = "localhost";
	boolean askingForIP = false;
	int port = 1000;

	public SimpleConnectionGame(Logger logger, String[] args, int ticksPerSecond, int paintTicksPerSecond, String title,
			int width, int height) {
		super(logger, args, ticksPerSecond, paintTicksPerSecond, title, width, height);
	}

	public static void main(String[] args) {
		us = new SimpleConnectionGame(new Logger(true), args, 60, 60, "Simple Connection Game", 400, 400);
