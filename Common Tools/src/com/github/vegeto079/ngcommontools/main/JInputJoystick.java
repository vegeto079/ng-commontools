 ...
 * need or add your own methods. <br>
 * JInput javadoc: {@link www.newdawnsoftware.com/resources/jinput/apidocs}<br>
 * More on the blog: {@link theuzo007.wordpress.com/2012/09/02/joystick-in
 * -java-with-jinput}
 * 
 * @author TheUzo007 and Nathan (slight modifying)
 */
...
public class JInputJoystick {
	 ...
	 *            What number controller to get. 1 for first controller found,
	 *            etc.
	 */
	public JInputJoystick(Controller.Type controllerType, int controllerNumber) {
		initialize();
		initController(controllerType, null, controllerNumber);
	}
	 ...
	 * 
	 * @param controllerType_1
	 *            Desired controller type.
	 * @param controllerType_2
	 *            Desired controller type.
	 * @param controllerNumber
	 *            What number controller to get. 1 for first controller found,
	 *            etc.
	 */
	public JInputJoystick(Controller.Type controllerType_1,
			Controller.Type controllerType_2, int controllerNumber) {
		initialize();
		initController(controllerType_1, controllerType_2, controllerNumber);
	}
	 ...
	 *            etc.
	 */
	private void initController(Controller.Type controllerType_1,
			Controller.Type controllerType_2, int controllerNumber) {
		...
		// Check for controller type 2
		if (controller == null)
			for (int i = 0; i < controllers.length && controller == null; i++) {
				if (controllers[i].getType() == controllerType_2) {
					if (controllerNumber > 1) {
						controllerNumber--;
						continue;
					}
	...
	private static ControllerEnvironment createDefaultEnvironment() {
		...
		try {
			...
			constructor = (Constructor<ControllerEnvironment>) Class.forName(
	 ...
	 * 
	 * @param identifier
	 *            Identifier that correspond to component.
	 ...
	 * 
	 * @param identifier
	 *            Identifier that correspond to component from which we need
	 *            value.
	 ...
	 * 
	 * @param index
	 *            Index of a button in array list.
